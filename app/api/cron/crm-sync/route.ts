...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    for (let i = 0; i < allDealRows.length; i += DEAL_BATCH) {
      ...
      // HubSpot autoritaire pour `dealstage` sur les deals natifs.
      // Les deals Diploma sont stockes sous un ID `dpl_*` distinct du hubspot_deal_id
      // natif : ils ne sont jamais touches par crm-sync (qui ne pull que des deals
      // HubSpot). La separation des cles garantit que Diploma reste autoritaire
      // pour ses propres lignes sans bloquer la sync HubSpot des deals natifs.
