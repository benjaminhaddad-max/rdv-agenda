...
export async function POST() {
  ...
  // Désactiver les closers qui ne sont plus dans la team HubSpot
  ...
  for (const cl of allClosers ?? []) {
    if (!cl.auth_id) continue
    const stillInTeam = teamUserIds.includes(String(cl.hubspot_user_id))
    const { data: authUser } = await db.auth.admin.getUserById(cl.auth_id)
    const isBanned = authUser?.user?.banned_until
      ? new Date(authUser.user.banned_until) > new Date()
      : false

    if (!stillInTeam && !isBanned) {
      await db.auth.admin.updateUserById(cl.auth_id, { ban_duration: '876000h' })
      banned.push(cl.email)
    } else if (stillInTeam && isBanned) {
      await db.auth.admin.updateUserById(cl.auth_id, { ban_duration: 'none' })
      unbanned.push(cl.email)
    }
  }
