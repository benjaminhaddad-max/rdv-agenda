...
async function getCloserTeamId(): Promise<string | null> {
...
export async function GET() {
  ...
  const result = (closers ?? []).map(cl => {
    const authUser = cl.auth_id ? authMap.get(cl.auth_id) : undefined
...
export async function PATCH(req: NextRequest) {
  ...
  const { data: cl, error: findErr } = await db
    .from('rdv_users')
    .select('id, name, email, auth_id, hubspot_user_id')
    .eq('id', userId)
    .single()

  if (findErr || !cl?.auth_id) {
    return NextResponse.json({ error: 'Closer introuvable ou sans compte' }, { status: 404 })
  }

  if (action === 'ban') {
    const { error: banErr } = await db.auth.admin.updateUserById(cl.auth_id, {
  ...
  if (action === 'reset-password') {
    ...
    const { error } = await db.auth.admin.updateUserById(cl.auth_id, { password: newPassword })
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    return NextResponse.json({ success: true, password: newPassword, email: cl.email })
  }
  ...
  if (action === 'impersonate') {
    ...
    const { data, error } = await db.auth.admin.generateLink({
      type: 'magiclink',
      email: cl.email,
      options: { redirectTo: `${siteUrl}/closer/${cl.name?.toLowerCase().replace(/\s+/g, '-')}` },
    })
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  ...
  // unban
  const { error: unbanErr } = await db.auth.admin.updateUserById(cl.auth_id, {
    ban_duration: 'none',
  })
  ...
  if (teamId) {
    const hsRes = await hubspotFetch('/settings/v3/users', {
      method: 'POST',
      body: JSON.stringify({ email: cl.email, primaryTeamId: teamId }),
    })
    if (hsRes.ok) {
      const hsData = await hsRes.json()
      await db
