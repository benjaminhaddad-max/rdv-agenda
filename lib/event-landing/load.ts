...
import { eventHasComms, eventOffersPublicInscriptionPage, eventOffersStandForm } from '@/lib/events-studio/config'
import { humanDescription, parseDateEnd } from '@/lib/events-studio/event-meta'
...
export type PublicFormPage =
  ...
  /** Formulaire lié à un salon externe : collecte sur stand (tablette + QR code), pas de pré-inscription publique. */
  | { kind: 'salon_stand'; form: PublicForm; event: EventLandingEvent; dateEnd: string | null }
  /** Formulaire lié à un événement sans page publique ni formulaire stand. */
...
export async function loadPublicFormPage(slug: string): Promise<PublicFormPage> {
  ...
  // Salons externes : pas de pré-inscription publique (on n’organise pas le
  // salon), mais un formulaire de collecte pour la tablette / le QR code du stand.
  if (!eventOffersPublicInscriptionPage(event)) {
    if (eventOffersStandForm(event)) {
      return {
        kind: 'salon_stand',
        form,
        event: {
        ...
        dateEnd: parseDateEnd(event.description),
      }
    }
