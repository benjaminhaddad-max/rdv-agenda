...
export default function CheckRdvCloserPanel({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      <div style={{ background: '#ffffff', border: '1px solid #e5ddc8', borderRadius: 16, width: '100%', maxWidth: 960, boxShadow: '0 24px 60px rgba(0,0,0,0.5)' }}>

        {/* Header */}
        <div style={{ padding: '20px 24px', borderBottom: '1px solid #e5ddc8', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <div style={{ fontWeight: 700, fontSize: 16, color: '#0e1e35', display: 'flex', alignItems: 'center', gap: 8 }}>
              <AlertTriangle size={16} style={{ color: '#E8C97B' }} />
              Check RDV Closer
            </div>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <button
              onClick={load}
              disabled={loading}
              style={{ background: '#f7f4ee', border: '1px solid #e5ddc8', borderRadius: 8, padding: '6px 10px', color: '#4a6070', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, fontFamily: 'inherit' }}
            >
        ...
        {/* Filter tabs */}
        <div style={{ padding: '16px 24px', borderBottom: '1px solid #e5ddc8', display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {FILTERS.map(f => {
            ...
            return (
              <button
                ...
                style={{
                  ...
                  background: isActive ? `${f.color}18` : '#f7f4ee',
                  border: `1px solid ${isActive ? `${f.color}55` : '#e5ddc8'}`,
                  borderRadius: 8, padding: '6px 12px',
                  color: isActive ? f.color : '#4a6070',
        ...
        <div style={{ padding: '10px 24px', background: 'rgba(204,172,113,0.05)', borderBottom: '1px solid #e5ddc8' }}>
          <p style={{ fontSize: 12, color: '#4a6070', margin: 0 }}>
        ...
        <div style={{ padding: '16px 24px', maxHeight: '62vh', overflowY: 'auto' }}>
          {loading && (
            <div style={{ textAlign: 'center', padding: '40px 0', color: '#4a6070', fontSize: 13 }}>
          ...
          {!loading && filtered.map(deal => {
            ...
            const catColors: Record<RdvPrisAuditDeal['category'], { label: string; color: string }> = {
              same_person:     { label: 'Même personne',   color: '#E8C97B' },
            ...
            return (
              <div key={deal.id} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                  ...
                  {/* Date */}
                  <div style={{ minWidth: 75, fontSize: 11, color: '#4a6070', flexShrink: 0 }}>
                    {date}
                  </div>

                  {/* Nom prospect */}
                  <div style={{ flex: 1, minWidth: 130, fontWeight: 700, fontSize: 13, color: '#0e1e35' }}>
                    {prospectName}
                  </div>

                  {/* Télépro */}
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 2, minWidth: 120 }}>
                    <span style={{ fontSize: 10, color: '#4a6070', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Télépro</span>
                    {deal.telepro_user ? (
                      ...
                      <span style={{ fontSize: 11, color: '#4a6070', fontStyle: 'italic' }}>
                  ...
                  {/* Propriétaire */}
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 2, minWidth: 120 }}>
                    <span style={{ fontSize: 10, color: '#4a6070', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Propriétaire</span>
                    {deal.owner_user ? (
                      <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                        ...
                        <div>
                          ...
                          <span style={{ fontSize: 10, color: '#4a6070', marginLeft: 4 }}>({deal.owner_user.role})</span>
                        </div>
                      </div>
                    ) : (
                      <span style={{ fontSize: 11, color: '#4a6070', fontStyle: 'italic' }}>
                  ...
                  <span
                    style={{
                      ...
                      borderRadius: 6, padding: '4px 9px', color: '#4a6070',
                      fontSize: 11, fontWeight: 600, flexShrink: 0,
                    }}
                  >
                ...
                <div style={{ display: 'flex', gap: 6, marginTop: 8, paddingTop: 8, borderTop: '1px solid #ffffff', flexWrap: 'wrap' }}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: 6, padding: '4px 11px', color: '#E8C97B',
                      fontSize: 11, fontWeight: 600, cursor: isActioning ? 'default' : 'pointer',
    ...
    {reassigningDeal && (
      ...
      >
        <div style={{
          background: '#ffffff', border: '1px solid #e5ddc8', borderRadius: 16,
          ...
          <div style={{ padding: '20px 24px', borderBottom: '1px solid #e5ddc8', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
            <div>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#E8C97B', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 6 }}>
                🔄 Réassigner le closer
              </div>
              <div style={{ fontSize: 16, fontWeight: 700, color: '#0e1e35' }}>
                {reassigningDeal.dealname.replace(/^RDV Découverte — /i, '').trim() || reassigningDeal.dealname}
              </div>
            </div>
            <button onClick={() => setReassigningDeal(null)} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: '#4a6070', padding: 4 }}>
              <X size={18} />
            </button>
          </div>

          <div style={{ padding: '12px 24px 16px', display: 'flex', flexDirection: 'column', gap: 6, maxHeight: '50vh', overflowY: 'auto' }}>
            <div style={{ fontSize: 11, fontWeight: 600, color: '#4a6070', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>
              Choisir le nouveau closer
            </div>
            {closers.length === 0 && <div style={{ color: '#4a6070', fontSize: 13 }}>Chargement…</div>}
            {closers.map((closer, idx) => {
              const COLORS = ['#C9A84C','#22c55e','#E8C97B','#a855f7','#06b6d4','#ef4444','#f97316']
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    border: `1px solid ${isSelected ? color : '#e5ddc8'}`,
                    borderRadius: 10, padding: '10px 14px',
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <span style={{ fontWeight: 600, fontSize: 13, color: '#0e1e35' }}>{closer.name}</span>
                      {isCurrent && (
                        <span style={{ background: 'rgba(204,172,113,0.15)', color: '#E8C97B', borderRadius: 5, padding: '1px 6px', fontSize: 10, fontWeight: 700 }}>
                          Actuel
                        </span>
                      )}
                    </div>
          ...
          <div style={{ padding: '14px 24px', borderTop: '1px solid #e5ddc8' }}>
            {reassignError && <div style={{ color: '#ef4444', fontSize: 12, marginBottom: 10 }}>{reassignError}</div>}
            <div style={{ display: 'flex', gap: 8 }}>
              <button
                onClick={() => setReassigningDeal(null)}
                style={{ flex: 1, background: 'transparent', border: '1px solid #e5ddc8', borderRadius: 8, padding: '9px', color: '#4a6070', fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}
              >
                Annuler
              </button>
              <button
                ...
                style={{
                  flex: 2, background: selectedCloserId ? '#C9A84C' : '#eaf0f6',
                  border: 'none', borderRadius: 8, padding: '9px',
                  color: selectedCloserId ? 'white' : '#4a6070', fontSize: 13,
