...
function BoardColumn({
  ...
  selectedDeals, onToggleSelect, onDragStartMulti,
}: {
  stageId: string
        ...
        if (raw) {
          try {
            ...
            if (dealIds.length === 1) {
              ...
              // Will be handled by parent via onBatchStageChange
              // We pass through onStageChange which parent intercepts
              // Actually we store the array and parent reads it
              const evt = new CustomEvent('batchDrop', { detail: { dealIds, newStage: stageId } })
              document.dispatchEvent(evt)
            }
          } catch {
            // Fallback: single dealId
